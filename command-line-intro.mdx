...
> _NOTE: All the examples here are for Mac/Linux users. If you are on Windows 10 or newer, you can use the [Windows Subsystem for Linux](https://docs.microsoft.com/en-us/windows/wsl/install-win10)._
...
<img
  src="https://i.stack.imgur.com/voqT3.jpg"
  alt="An image of an old punch card machine"
  width="500px"
/>
...
> _[Here is a really good thread](https://softwareengineering.stackexchange.com/questions/262723/why-were-punch-cards-used-for-programming) talking about why punched cards were used for early computing_
...
<Image
  src="https://raw.githubusercontent.com/mikelbrierly/blog-posts/main/images/terminalPlain.png"
  alt="Terminal window that reads 'mhowart1 is on tatooine in ~', and then a blinking cursor."
/>
...
_(if you don't see that, type `cd` and hit enter)_
...
Let's make a _directory_ now. We'll call it "stomach".
...
At this point, we are _inside_ the stomach of the tauntaun. but what if we wanted to go back **multiple** directories?
...
> `~` = _home directory_ > `/` = _root directory_
...
![Screen Shot of a string saying grep "created" test.txt, and then an output from the terminal with the word "created" highlighted.](https://i.imgur.com/WnBZAxJ.png)
...
![Screen Shot of the command cat test.txt | grep "created" with the output showing "created" highlighted](https://i.imgur.com/BMDGYRN.png)
...
Now if we take that output and **pipe** it _to_ `grep`, we can search its output.
...
_If we wanted to see lines around our match, we could use a `flag` of `-C`:_
...
_Remember that `|` can be used with anything that gives output. (`grep` is just an example)_
...
For example, running `rm tauntaun` will tell you `rm: tauntaun: is a directory`. So we need to use the `-r` _flag_.
...
- Download [this repo](https://github.com/mikelbrierly/command-line) to your local machine:
